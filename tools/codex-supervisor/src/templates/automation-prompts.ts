...
export function buildPlannerAutomationPrompt(): string {
  return joinLines([
    ...
    "- If a worker, reviewer, or sprint cycle leaves a next-step suggestion that still fits the approved goal, fold it into proposal or task rebalancing.",
...
export function buildReviewerAutomationPrompt(): string {
  return joinLines([
    ...
    "- If the change needs follow-up but stays inside the approved goal, mark it `followup_required` and leave a concise next-step suggestion.",
...
export function buildReporterAutomationPrompt(): string {
  return joinLines([
    ...
    "- Include any recorded next-step suggestion when it stays inside the approved goal boundary.",
...
export function buildSprintAutomationPrompt(): string {
  return joinLines([
    ...
    "- If the completed task suggests a safe follow-up inside the approved goal set, record the suggestion and let planning or the next loop pick it up.",
