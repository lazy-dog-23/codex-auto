...
- `create_successor_goal` is disabled by default. It only becomes automation-ready when `autonomy/decision-policy.json` defines an authorized long-running charter and allows one minimal successor goal per completed boundary after all approved work is complete; policy quotas remain as a safety throttle. The write command also enforces the same bound-thread and `status` / `decide` gates at the CLI boundary.
